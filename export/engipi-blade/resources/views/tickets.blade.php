...
@section('title', "Engipi — تیکت‌های پشتیبانی")
...
<div class="app">
  ...
  <div class="main">
    ...
    <div class="content">
      <div class="page-head">
        <div><h2>تیکت‌های پشتیبانی</h2><p>سؤال یا مشکلی داری؟ تیم Engipi همراه توست.</p></div>
...
<script>
  ...
  const CONV = {
    'TK-۱۰۲۴': {
      ...
      msgs: [
        ...
        ['agent','پ','تیم پشتیبانی Engipi','سلام علی عزیز، ممنون از گزارشت. کد پیگیری پرداخت را بررسی کردیم؛ تراکنش با موفقیت ثبت شده و در حال هماهنگی با درگاه هستیم. حداکثر تا ۲ ساعت آینده وضعیت پروژه به‌روزرسانی می‌شود.','۱۴۰۴/۰۳/۱۸ — ۱۰:۴۱'],
  ...
  function openTicket(id) {
    renderList(id);
    const c = CONV[id] || { title: TICKETS.find(t=>t[0]===id)[1], dept: TICKETS.find(t=>t[0]===id)[4], status: TICKETS.find(t=>t[0]===id)[3], msgs: [['agent','پ','تیم پشتیبانی Engipi','این تیکت بسته شده است. در صورت نیاز تیکت جدیدی ثبت کنید.','—']] };
