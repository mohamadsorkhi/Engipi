...
@section('title', "EngPis — مارکت‌پلیس تخصصی پروژه‌های مهندسی")
...
<nav class="nav" id="nav">
  <div class="bp-container row">
    <a class="brand" href="#"><span class="a">Eng</span>Pis</a>
    <ul class="nav-links">
    ...
    <div class="nav-act">
      <a class="nav-login" href="/auth">ورود</a>
      <a class="bp-btn bp-btn--primary bp-btn--sm" href="/auth">ثبت‌نام رایگان</a>
    </div>
  </div>
</nav>
...
<header class="hero">
  ...
  <div class="bp-container hero-grid">
    <div>
      ...
      <div class="hero-cta">
        <a class="bp-btn bp-btn--primary bp-btn--lg" href="/projects/create"><i class="ri-add-circle-line"></i>ثبت پروژه</a>
        <a class="bp-btn bp-btn--ghost-d bp-btn--lg" href="/dashboard?v=matched"><i class="ri-search-line"></i>جستجوی متخصص</a>
      </div>
      <div class="trust">
...
<section class="stats">
  <div class="bp-container stats-grid">
    <div class="statbox"><div class="n">+۵۰۰</div><div class="l">پروژه ثبت شده</div></div>
    <div class="statbox"><div class="n">+۱۲۰۰</div><div class="l">متخصص فعال</div></div>
    <div class="statbox"><div class="n">+۳۵۰</div><div class="l">کارفرمای راضی</div></div>
    <div class="statbox"><div class="n">۱۵+</div><div class="l">حوزه تخصصی</div></div>
  </div>
</section>
...
<section class="bp-section" style="padding-top:0">
  <div class="bp-container">
    <div class="cta-wrap">
      ...
      <div class="cta-row">
        <a class="bp-btn bp-btn--primary bp-btn--lg" href="/projects/create"><i class="ri-add-circle-line"></i>ثبت پروژه مهندسی</a>
        <a class="bp-btn bp-btn--ghost-d bp-btn--lg" href="/dashboard?v=matched"><i class="ri-briefcase-4-line"></i>جستجوی کار مهندسی</a>
      </div>
    </div>
  </div>
</section>
...
<footer class="footer">
  <div class="bp-container">
    <div class="fgrid2">
      ...
      <div><h6>EngPis</h6><ul><li><a href="#">درباره ما</a></li><li><a href="#">قوانین</a></li><li><a href="#">تماس</a></li><li><a href="#">وبلاگ</a></li></ul></div>
      <div><h6>کارفرمایان</h6><ul><li><a href="#">ثبت پروژه</a></li><li><a href="#">نحوه کار</a></li><li><a href="#">تعرفه‌ها</a></li><li><a href="#">سوالات</a></li></ul></div>
      <div><h6>متخصصان</h6><ul><li><a href="#">ثبت‌نام</a></li><li><a href="#">یافتن پروژه</a></li><li><a href="#">راهنما</a></li><li><a href="#">سوالات</a></li></ul></div>
    </div>
    <div class="fbottom">
...
<script>
  ...
  const DOMAINS = [
    ['ri-flashlight-line','--bp-c-amber','--bp-tint-amber','مهندسی برق','مدار، قدرت، الکترونیک'],
    ['ri-settings-4-line','--bp-blue','--bp-tint-blue','مهندسی مکانیک','طراحی، ساخت، دینامیک'],
    ['ri-building-2-line','--bp-teal','--bp-tint-teal','مهندسی عمران','سازه، ژئوتکنیک، راه'],
    ['ri-code-s-slash-line','--bp-c-green','--bp-tint-green','مهندسی کامپیوتر','نرم‌افزار، هوش مصنوعی'],
    ['ri-flask-line','--bp-c-red','--bp-tint-red','مهندسی شیمی','فرآیند، پلیمر، پتروشیمی'],
    ['ri-bar-chart-grouped-line','--bp-c-purple','--bp-tint-purple','مهندسی صنایع','بهینه‌سازی، لجستیک'],
    ['ri-flight-takeoff-line','--bp-c-sky','--bp-tint-sky','مهندسی هوافضا','آیرودینامیک، پیشرانش'],
    ['ri-microscope-line','--bp-c-orange','--bp-tint-orange','سایر رشته‌ها','معدن، متالورژی، محیط زیست'],
  ];
  const FEATURES = [
  ...
  const tile = (ic,c,t) => `<div class="bp-icon-tile" style="background:var(${t});color:var(${c})"><i class="${ic}"></i></div>`;

  function renderSteps(tab) {
    document.getElementById('steps').innerHTML = STEPS[tab].map((s,i) => `
  ...
  document.getElementById('dgrid').innerHTML = DOMAINS.map(d => `
    <div class="bp-card domain">${tile(d[0],d[1],d[2])}<h5>${d[3]}</h5><small>${d[4]}</small></div>`).join('');
  document.getElementById('fgrid').innerHTML = FEATURES.map(f => `
    <div class="bp-card feature">${tile(f[0],f[1],f[2])}<h5>${f[3]}</h5><p>${f[4]}</p></div>`).join('');
  document.getElementById('tgrid').innerHTML = TESTI.map(t => `
    <div class="bp-card testi"><div class="stars">★★★★★</div><p>${t[1]}</p>
      <div class="person"><div class="av" style="background:var(${t[0]})">${t[4]}</div>
        <div><div class="nm">${t[2]}</div><div class="rl">${t[3]}</div></div></div></div>`).join('');

  document.querySelectorAll('.tabbtn').forEach(b => b.addEventListener('click', () => {
    document.querySelectorAll('.tabbtn').forEach(x => x.classList.remove('active'));
    b.classList.add('active'); renderSteps(b.dataset.tab);
  }));
  renderSteps('employer');
</script>
@endverbatim
@endpush
