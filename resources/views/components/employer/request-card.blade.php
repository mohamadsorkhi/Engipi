    ...
    <div class="card-body">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            ...
            <div class="text-md-end">
                <a
                    href="{{ route('user.projects.show', $request->project) }}"
                    class="d-block fw-medium text-primary mb-1"
                >
                ...
                <div class="text-muted fs-12">
                    ...
                    <span class="d-inline-block">
                        {{ \Morilog\Jalali\Jalalian::fromFormat('Y-m-d H:i:s', $request->created_at)->ago() }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="mb-4">
            @if ($request->message)
                <div class="p-3 bg-light rounded border border-light">
                    <h6 class="fs-13 fw-semibold text-muted mb-2">
                        پیام متخصص:
                    </h6>
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3 pt-3 border-top border-light">
            ...
            <div class="d-flex gap-2">
                @if ($request->status === 'pending')
                    <form
                        action="{{ route('user.requests.accept', $request) }}"
                        method="POST"
                    >
                        @csrf
                        ...
                        <button
                            type="submit"
                            class="btn btn-success btn-sm ajax-submit"
                        >
                    ...
                    <form
                        action="{{ route('user.requests.reject', $request) }}"
                        method="POST"
                    >
                        @csrf
                        ...
                        <button
                            type="submit"
                            class="btn btn-danger btn-sm ajax-submit"
                        >
                ...
                @elseif ($request->status === 'rejected' || $request->status === 'accepted')
                    <form
                        action="{{ route('user.requests.revert', $request) }}"
                        method="POST"
                    >
                        @csrf
                        ...
                        <button
                            type="submit"
                            class="btn btn-soft-warning btn-sm ajax-submit"
                        >
